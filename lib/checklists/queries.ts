...
export class ChecklistDepartmentMismatchError extends Error {}
...
export type ChecklistDepartmentGroup<T> = {
  department: (typeof CHECKLIST_DEPARTMENTS)[number];
  label: string;
  items: T[];
};
...
export async function listItemsGroupedByDepartment(
  branchId: string,
  userId: string
): Promise<ChecklistDepartmentGroup<ChecklistItemRow>[]> {
  return withTenantContext({ userId, branchId }, async (tx) => {
    const items = await tx.openingChecklistItem.findMany({
      where: { branchId },
...
export async function getChecklistItemById(
  ...
  return withTenantContext({ userId, branchId }, async (tx) => {
    const item = await tx.openingChecklistItem.findFirst({ where: { id, branchId } });
    if (!item) return null;
    return {
      id: item.id,
      title: item.title,
...
export type TodaysChecklistItem = ChecklistItemRow & {
  isCompleted: boolean;
 ...
 */
export async function getTodaysChecklist(
  branchId: string,
  userId: string,
  timezone: string,
  restrictToDepartment?: ChecklistDepartmentValue | null,
  date?: Date
): Promise<ChecklistDepartmentGroup<TodaysChecklistItem>[]> {
...
export async function getProgressByDepartment(
  ...
  const groups = await getTodaysChecklist(branchId, userId, timezone, undefined, date);
 ...
 *
 * `restrictToDepartment` mirrors getTodaysChecklist's semantics: `undefined` for
 * Owner/Manager (no restriction — can toggle any item), a specific department for a
 * staff member (can only toggle items in that department), or `null` for a staff
 * member with no department assigned (can't toggle anything).
 */
export async function toggleTodaysCompletion(
  ...
  return withTenantContext({ userId, branchId }, async (tx) => {
    const item = await tx.openingChecklistItem.findFirst({ where: { id: itemId, branchId } });
    if (!item) throw new ChecklistItemNotFoundError("Checklist item not found.");

    if (restrictToDepartment !== undefined && item.department !== restrictToDepartment) {
      throw new ChecklistDepartmentMismatchError("This checklist item isn't in your department.");
    }

    const existing = await tx.openingChecklistCompletion.findUnique({
      where: { itemId_date: { itemId, date: today } },
    });

    if (existing) {
      await tx.openingChecklistCompletion.delete({ where: { id: existing.id } });
      return { isCompleted: false };
    }

    await tx.openingChecklistCompletion.create({
...
export async function moveChecklistItem(
  ...
  await withTenantContext({ userId, branchId }, async (tx) => {
    ...
    const siblings = await tx.openingChecklistItem.findMany({
    ...
    await tx.openingChecklistItem.update({
      where: { id: item.id },
