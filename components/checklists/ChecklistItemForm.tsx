...
export function ChecklistItemForm({ branchSlug, itemId, initialValues }: Props) {
  ...
  const endpoint = isEditing
    ? `/api/branches/${branchSlug}/checklist-items/${itemId}`
    : `/api/branches/${branchSlug}/checklist-items`;
  ...
  async function onSubmit(event: FormEvent) {
    ...
    try {
      ...
      router.push(`/office/${branchSlug}/checklists`);
      router.refresh();
    } finally {
      setLoading(false);
    }
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="flex gap-2">
        ...
        <Button
          type="button"
          variant="secondary"
          onClick={() => router.push(`/office/${branchSlug}/checklists`)}
        >
          Cancel
        </Button>
      </div>
